...
rlt$symbol<-as.character(ENSG2Symbol(as.character(rownames(rlt))))

memo="cldn6.dge"

# save dge to csv
write.table(rlt,file=paste(memo,"tcga.pancancer.smd.meta.pvalue.txt",sep=""),sep="\t",quote=F,col.names = NA,row.names = T)
write.csv(rlt,file=paste(memo,"tcga.pancancer.smd.meta.pvalue.csv",sep=""),quote=F)

# install.packages("CMplot")
library("CMplot")
ensg<-read.table("~/hpc/db/hg19/ENSG.hg19.bed")
output<-data.frame(ensg[match(unlist(lapply(strsplit(rownames(rlt),"[.]"),function(x) x[1])),ensg[,5]),],rlt)
